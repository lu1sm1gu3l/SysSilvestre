 ...
 * @author Katya
 */
public class CDNotaCredito {

     public static CENotaCredito CosultarNotaCreditoPorId(long id)
    {
            ...
            CENotaCredito oCENotaCredito=null;
                if(cnn!=null)
                {
                     CallableStatement cs= null;
                      ...
                      cs = cnn.prepareCall("{CALL  VTASPRCNSNotaCreditoPorCodigo(?,?,?,?,?)}");
                     cs.setInt(1, 1);
                     cs.setString(2,null);
                     cs.setLong(3, id);
                     cs.setInt(4, 0);
                     ...
                     ResultSet rs=cs.executeQuery();
                      while(rs.next())
                      {
                       ...
                       oCENotaCredito.setLstCENotaCreditoDetalle(ListarNotaCreditoDetalle(oCENotaCredito.getIdNotaCredito(),1));}
     ...
     public static List<CENotaCreditoDetalle> ListarNotaCreditoDetalle(long IdNotaCredito,int idCns)
    {
        Connection cnn= ConexionBD.obtenerConexion();
        try
        {
            ...
            CENotaCreditoDetalle oCENotaCreditoDetalle=null;
                ...
                {
                     CallableStatement cs= null;
                     cs = cnn.prepareCall("{CALL  VTASPRCNSNotaCreditoDetalle(?,?)}");
     ...
     public static String[] InsNotaCredito(CENotaCredito oCENotaCredito,CEComprobanteVentaMatriz oCEComprobanteVentaMatriz,
        ...
        try
          {
            ...
            String sql= "{call VTASPRINSNotaCredito(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
            ...
            sentencia.setLong(20, 0);// id ingreso
            sentencia.setString(21, null);// numIngreso
            ...
            {
                String sql_= "{call VTASPRINSNotaCreditoDetalle(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
        ...
        try
          {
            ...
            String sql= "{call VTASPRUPDNotaCredito(?,?,?,?,?,?,?,?,?,?,?,?)}";
            sentencia = conexion.prepareCall(sql);
            sentencia.setLong(1, oCENotaCredito.getIdNotaCredito());
...
public static int AnularNotaCredito(CENotaCredito oCENotaCredito)
    {
       ...
       conexion = ConexionBD.obtenerConexion();//5 elimina
        try
          {
            CallableStatement sentenciaNotaCredito;
            CallableStatement sentenciaIngreso;
            CallableStatement sentenciaIngresoDet;
            conexion.setAutoCommit(false);
            conexion.setTransactionIsolation(Connection.TRANSACTION_READ_UNCOMMITTED);

            String sql= "{call VTASPRINSNotaCreditoEstado(?,?,?,?,?,?,?)}";
            sentenciaNotaCredito = conexion.prepareCall(sql);
            sentenciaNotaCredito.setLong(1, oCENotaCredito.getIdNotaCredito());
            sentenciaNotaCredito.setInt(2, 4);
            sentenciaNotaCredito.setInt(3, oCENotaCredito.getIdUsuario());
            sentenciaNotaCredito.setInt(4, oCENotaCredito.getIdEmpleado());
            sentenciaNotaCredito.setString(5,oCENotaCredito.getUltimaObservacion());
            sentenciaNotaCredito.setLong(6, oCENotaCredito.getIdComprobanteVentaRef());
            sentenciaNotaCredito.setLong(7, oCENotaCredito.getIdConcepto());
            sentenciaNotaCredito.executeUpdate();

            if(oCENotaCredito.getIdConcepto()==2)
            {
                String sqlIng= "{call AlMSPRINSAnularIngresoPorNotaCredito(?,?,?,?,?,?,?)}";
                sentenciaIngreso = conexion.prepareCall(sqlIng);
                sentenciaIngreso.setLong(1, oCENotaCredito.getIdNotaCredito());
                sentenciaIngreso.setLong(2,0);//idIngreso
                sentenciaIngreso.setFloat(3,oCENotaCredito.getIdUsuario());
                sentenciaIngreso.setFloat(4,oCENotaCredito.getIdEmpleado());
                sentenciaIngreso.executeUpdate();

                long pIdIngreso=sentenciaIngreso.getLong(2);

            for (CENotaCreditoDetalle oCENotaCreditoDetalle : oCENotaCredito.getLstCENotaCreditoDetalle())
            {
                String sqlIngDet= "{call ALMSPRUPDAnularIngresoProductoDetalle(?,?,?,?,?,?)}";
                sentenciaIngresoDet = conexion.prepareCall(sqlIngDet);
                sentenciaIngresoDet.setLong(1,oCENotaCreditoDetalle.getIdProducto());
                sentenciaIngresoDet.setFloat(2,oCENotaCreditoDetalle.getIdAlmacen());
                sentenciaIngresoDet.setFloat(3,oCENotaCreditoDetalle.getCantidadNueva());
                sentenciaIngresoDet.setFloat(4,oCENotaCreditoDetalle.getIdUnidadMedidaVenta());
                sentenciaIngresoDet.setLong(5,4);
                sentenciaIngresoDet.setInt(6,0);
                sentenciaIngresoDet.executeUpdate();
             }
 ...
 public static List<CENotaCredito> ListarNotaCredito(String pDato,int opcion,long id,int idestado,String pFechaFin)
    {
        Connection cnn = ConexionBD.obtenerConexion();
        try
        {
        ...
        {
            System.out.print(ex);
            JOptionPane.showMessageDialog(null, "Error al cargar ela Nota de Credito ");
            return null;
        }
        finally
        {
