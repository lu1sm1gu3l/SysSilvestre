...
public class DlgAnularNotaCredito extends DialogoPadre
{
    boolean sinoDevolucion=false;
    CENotaCredito oCENotaCredito;
    public DlgAnularNotaCredito(java.awt.Frame parent, boolean modal,CENotaCredito oCENotaCredito,boolean sinoDevolucion)
    {
        super(parent, modal);
        initComponents();
        this.setLocationRelativeTo(null);
        this.oCENotaCredito=oCENotaCredito;
        this.sinoDevolucion=sinoDevolucion;
        if(sinoDevolucion)
        {
            LblTitulo.setText("Anular Comprobante Devolución");
        }
        else
        {
        ...
        TxtNumPedido.setText(oCENotaCredito.getNumeroNotaCredito());
    ...
    private void BtnAnularActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnAnularActionPerformed
     ...
     oCENotaCredito.setUltimaObservacion(TxObservacion);

     int rpt=0;
     if(sinoDevolucion)
     {
         rpt=CCComprobanteDevolucion.AnularComprobanteDevolucion(oCENotaCredito);
     }
        else{
        rpt=CCNotaCredito.AnularNotaCredito(oCENotaCredito);
        }
     if(rpt==1)
     {
         isAnulado=true;
