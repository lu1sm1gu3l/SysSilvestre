...
public class CLOrdenCompra
{
   ...
   public int colCantidad=4;
    ...
    JLabel LblMontoTotalPagar;

    private JTextField TxtDescuento;
    private JComboBox CbxTipoDescuentoOrdenCompra;


    public CLOrdenCompra(JTable oTabla,JLabel lblsubtSinDesc,
                    ...
                    JTextField txtDescuento,JComboBox CbxTipoDescuentoOrdenCompra,JLabel LblPercepcion,JCheckBox percepcion)
    {
        this.oTabla = oTabla;
        ...
        this.CbxTipoDescuentoOrdenCompra=CbxTipoDescuentoOrdenCompra;
...
private void calcularItem()
 ...
 public void setCalcularImport(boolean pbol)
   {
     this.calcularImport=pbol;
 }
        ...
        if(calcularImport)
      {
        if(sinodescuento){
              ...
              LblDescuentoEquivalente.setText("0.00");
                  ...
                  CETipoDescuento oDescuento=(CETipoDescuento)CbxTipoDescuentoOrdenCompra.getSelectedItem();
                  if(oDescuento!=null)
                  {
        ...
        if(oModelo.getRowCount()>0){
            for(int i=0;i<oModelo.getRowCount();i++)
                ...
                float imporSubtparaIvg=0;
                 ...
                 CEOrdenCompraDetalle oCEOrdenCompraDetalle=(CEOrdenCompraDetalle)oTabla.getValueAt(i, colCodigo);
                        if(oCEOrdenCompraDetalle!=null){
                        if(oCEOrdenCompraDetalle.isSinoImpuesto())
                        {
                ...
                if(oModelo.getValueAt(i, colImporSinDesc)!=null){
                    ImporteSinDescuento=VerificadorDeTxt.convertFloat(oModelo.getValueAt(i, colImporSinDescConIgv).toString());
                    }
                ...
                if(oDescuento!=null)
                     {                            
                            ...
                            if(oModelo.getValueAt(i, colDescuento)!=null){
                            DescuentoenImporte=Float.parseFloat(oModelo.getValueAt(i, colDestValores).toString());
    ...
    public void calcularImportes()
    {
       if(calcularImport)
       {
        ...
        if(sinigv)
        {
           ...
           float NetoConigv=SubtotalNeto+Montoigv;
  ...
  try {
    if(fil!=-1){
         ...
         oTabla.setValueAt(CLRedondear.RedondearString(ImporsinDesSinIgv,2), fil, colImporSinDesc);
            ...
            {
                CEOrdenCompraDetalle oCEOrdenCompraDetalle=(CEOrdenCompraDetalle)oTabla.getValueAt(fil, colCodigo);
            ...
            {
                System.out.println("Error CLOrdenCompra-CalcularCelda"+e);
            }
    }
    ...
    if(fil!=-1){
         ...
         float ImporsinDescTemp=Float.parseFloat(ImporSinDescStr);
          ...
          //  oTabla.setValueAt(ImporSinDescStr, fil, colImporSinDesc);//sin
            ...
            {
                 System.out.println("Error CLOrdenCompra-CalcularPrecioDesdeImporte"+e);
            }
    }
  ...
  try {
    if(fil!=-1){
         if(col==colImporSinDescConIgv)
            {
             float descuentodet=0;
             float ImporteconIgv=0;
             float ImportesinIgv=0;
             float cantidad=0;
             float precioConIgv=0;
         ...
         float ImporsinDescTemp=Float.parseFloat(ImporSinDescStr);
            ...
            if(oTabla.getValueAt(fil, colCodigo)!=null)
            {
                CEOrdenCompraDetalle oCEOrdenCompraDetalle=(CEOrdenCompraDetalle)oTabla.getValueAt(fil, colCodigo);
                String imporConDesc=(CLRedondear.RedondearString((ImporsinDescTemp)-descuentodet,2));
                float precioConDesc=((ImporsinDescTemp)-descuentodet)/cantidad;
            ...
            {
                System.out.println("Error CLOrdenCompra-CalcularPrecioDesdeImporteConIGV"+e);
            }
    }
...
{     
        ...
        CEOrdenCompraDetalle oCEOrdenCompraDetalle= (CEOrdenCompraDetalle)oTabla.getValueAt(fila, colCodigo);
         if(oCEOrdenCompraDetalle!=null){
